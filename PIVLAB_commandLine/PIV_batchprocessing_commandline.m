...
for folder_idx = 1:numel(run_folders)
    ...
    %% Save all processing settings as a JSON file next to the results
    % One JSON per run folder, sitting next to PIVlab_results_uncalibrated.mat,
    % grouping the preprocessing, PIV analysis and postprocessing settings so
    % the exact parameters used to produce the .mat are recoverable later.
    settings = struct();
    settings.meta = struct( ...
        'run_folder', run_folder, ...
        'file_pattern', file_pattern, ...
        'num_pairs', num_pairs, ...
        'results_file', 'PIVlab_results_uncalibrated.mat', ...
        'created', datestr(now, 'yyyy-mm-ddTHH:MM:SS'));
    ...
    settings.piv = struct( ...
        ...
        'delta_diff_min', delta_diff_min, ...
        'limit_peak_search_area', limit_peak_search_area);
    settings.postprocessing = struct( ...
        'calu', calu, ...
        'calv', calv, ...
        'valid_vel', valid_vel, ...
        'do_stdev_check', do_stdev_check, ...
    ...
    file_settings = fullfile(results_folder, 'PIVlab_results_uncalibrated_settings.json');
    try
        json_txt = jsonencode(settings, 'PrettyPrint', true);
    catch
        json_txt = jsonencode(settings);   % older MATLAB without PrettyPrint
    end
    fid = fopen(file_settings, 'w');
    if fid == -1
        warning('Could not open %s for writing.', file_settings)
    else
        fwrite(fid, json_txt, 'char');
        fclose(fid);
        disp(['Saved settings to: ' file_settings])
    end
end
