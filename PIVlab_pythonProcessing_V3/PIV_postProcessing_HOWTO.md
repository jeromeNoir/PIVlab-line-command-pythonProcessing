...
| `single_KineticEnergy.ipynb` | The full `batch_KineticEnergy` work for **one** run (time series + both spectra, combined `.npz`); optionally appends its row to the single `KineticEnergy_summary`. |
| `single_KineticEnergyFFT.ipynb` | Viewer: reads one combined `KineticEnergy` `.npz` and (re)draws its Ek spectrum. |
...
3. `batch_KineticEnergy` computes the kinetic-energy time series **and** both of
   its FFT spectra in a single pass. `batch_VelocityFFT` is independent.
...
The `batch_KineticEnergy` computation for **one** `RUN_DIR`: same combined
`KineticEnergy_<region>.npz`, same two-panel figure, and — when `UPDATE_SUMMARY`
is `True` — inserts/replaces this run's row in the single
`KineticEnergy_summary_<region>.csv` (same schema as the batch). `PER_POINT`
toggles the `⟨FFT(Ek)⟩` computation; `REPROCESS` guards overwriting existing
outputs.

### `single_KineticEnergyFFT.ipynb` / `.py`

A viewer. Reads one combined `KineticEnergy_<region>.npz` and redraws the Ek time
series + both spectra (recomputing from the stored series / re-read `.mat`), for
quick inspection or a re-styled figure. Writes no summary.

### `single_VelocityFFT.ipynb` / `.py`
