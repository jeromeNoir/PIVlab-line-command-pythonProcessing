...
# 
# Runs, for **one** run folder, exactly what `batch_KineticEnergy` does per run,
# in a single pass:
# 
# 1. the **kinetic-energy time series** over the region, `<Ek>(t)`, and
# 2. **both FFT spectra** of the kinetic energy -- `FFT(<Ek>)` (average, then FFT)
#    and `<FFT(Ek)>` (FFT at each point, then average) --
# 
# saved together in one combined `PostProcessing/KineticEnergy_<region>.npz`, with a
# two-panel figure `PostProcessing/KineticEnergy_FFT_<region>.png` (both spectra drawn).
# 
# Set `REGION` to `'ROI'` (crop to `PTS_ROI`) or `'full'` (whole field); the choice is
# written into every output filename, so the two never collide.
# 
# It can optionally add/refresh this run's row in the region's single shared summary
# `KineticEnergy_summary_<region>.csv` at the dataset root (`UPDATE_SUMMARY`). Only the
# `<FFT(Ek)>` spectrum is tabled (peak + amplitudes at `f_lib` and `2*f_lib`).
# 
# ### Configuration comes from `param_postProcessing.json`
# The calibration, ROI and folder-name conventions are read from the dataset's parameter
# file via the shared library, so this run uses **the same constants as the batch tools**.
...
from piv_postprocessing_lib import (topography_arrangement, load_piv, region_fields, region_tag,
                        ...
                        parse_run_name, amp_at_freq, amp_from_rfft, compute_fft,
...
# Per-run outputs and the single combined summary -- the region tag is appended
# below, so e.g. KineticEnergy_ROI.npz (time series + both FFT spectra) and
# KineticEnergy_summary_ROI.csv.
KE_NPZ_STEM   = 'KineticEnergy'
...
REPROCESS = True
# Add/refresh this run's row in the combined KineticEnergy summary. False ->
# the summary is left untouched (only the per-run npz + figure are written).
UPDATE_SUMMARY = True
...
# ## 3. Kinetic-energy time series over the region
# 
# Identical to `batch_KineticEnergy`'s per-run step: load, calibrate, crop to the region,
# then `Ek(t) = 0.5 * <u^2 + v^2>` per frame. The combined `.npz` is written in the FFT
# cell below, once both spectra have been computed.
...
# The combined npz (time series + both FFT spectra) is written in the FFT cell
# below, once both spectra have been computed.
print('run     : %s' % name)
...
# 
# Both spectra: `FFT(<Ek>)` (average, then FFT) and, if `PER_POINT`, `<FFT(Ek)>` (FFT
# per point, then average). Only `<FFT(Ek)>` is kept in the summary. Writes the combined
# `KineticEnergy_<region>.npz` (time series + both spectra) and `KineticEnergy_FFT_<region>.png`.
...
f_peak_avg, amp_peak_avg = peak_freq(freq, amp)

# (2) <FFT(Ek)>: FFT at each ROI point, then average -- kept in the summary.
freq_pt = amp_pt = None
f_peak = amp_peak = amp_flib = amp_2flib = np.nan
npoints = np.nan
if PER_POINT:
    Ek_pt = 0.5 * (Ur ** 2 + Vr ** 2).reshape(-1, nframes)
    freq_pt, amp_pt, npoints = compute_fft_per_point(Ek_pt, fps, detrend=DETREND)
    f_peak, amp_peak = peak_freq(freq_pt, amp_pt)
    amp_flib = amp_at_freq(freq_pt, amp_pt, flib_hz)
    amp_2flib = amp_at_freq(freq_pt, amp_pt, 2.0 * flib_hz)

# One combined npz: time series + both spectra + the kept <FFT(Ek)> scalars.
_amp_mean_store = amp_pt if amp_pt is not None else np.full_like(amp, np.nan)
if os.path.isfile(ke_npz) and not REPROCESS:
    print('[skip] %s exists and REPROCESS is False -- not overwritten'
          % os.path.basename(ke_npz))
else:
    ...
    np.savez(ke_npz,
             ...
             nframes=nframes, t=t, Ek_frame=Ek_frame, Ek_frame_star=Ek_frame_star,
             ...
             amp_fft_of_mean=amp, amp_mean_of_fft=_amp_mean_store,
             f_peak=f_peak, amp_peak=amp_peak, amp_flib=amp_flib, amp_2flib=amp_2flib)
    print('wrote', ke_npz)

# Both a raw and a normalized figure (both spectra drawn).
...
print('peak FFT(<Ek>)  : %.4g Hz  (amp %.3e)' % (f_peak_avg, amp_peak_avg))
if PER_POINT:
    print('peak <FFT(Ek)>  : %.4g Hz  (amp %.3e, from %s points)  [kept in summary]'
          % (f_peak, amp_peak, '-' if not np.isfinite(npoints) else int(npoints)))
plt.show()
...
# One combined row: folder metadata, time-series stats and the <FFT(Ek)>
# quantities (peak + amplitudes at f_lib and 2*f_lib). Same schema as the batch.
row = {'run': name, 'run idx': run_idx, 'region': tag, 'processed': True,
       'frot_Hz': frot_hz, 'flib_Hz': flib_hz, 'dphi_deg': dphi_deg,
       'fstar': fstar, 'flib_star': fstar, 'calibrated': ok,
       'dt_vel_s': dt_vel, 'fps_Hz': fps, 'nframes': nframes, 'npoints': npoints,
       'mean_Ekin': mean_Ekin, 'std_Ekin': std_Ekin,
       ...
       'npz': ke_npz, 'figure': fig_file}
row.update(dl)

base_dir = os.path.dirname(RUN_DIR.rstrip('/'))     # dataset root, where the summary lives
ke_summary_csv = os.path.join(base_dir, '%s_%s.csv' % (KE_SUMMARY, tag))

if UPDATE_SUMMARY:
    update_summary(ke_summary_csv, row)
    print('updated %s summary in %s' % (tag, base_dir))
