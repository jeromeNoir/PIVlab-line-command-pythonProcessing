...
# 
# A viewer. It reads one combined per-run `KineticEnergy_<region>.npz` (produced by
# `batch_KineticEnergy` / `single_KineticEnergy`, containing `t`, `Ek_frame` and the
# PIV sampling frequency `fps`), computes the **FFT** of `Ek(t)`, and draws two panels:
# 
# 1. the kinetic-energy time series `Ek(t)`;
# 2. its amplitude spectrum `|FFT|` versus frequency.
# 
# The one-sided amplitude is normalised as `|FFT| * 2 / N` so a pure tone reads as its
# physical amplitude (DC and Nyquist bins keep the `1/N` scaling). By default the mean is
# removed first. If the libration frequency can be parsed from the run name, dashed guides
# are drawn at `flib` and `2*flib`.
...
NPZ_STEM = "KineticEnergy"   # + _<region>.npz
...
# run folder (its `PostProcessing/KineticEnergy_<region>.npz` is used).
...
REGION  = 'ROI'       # 'ROI' or 'FULL' -- which KineticEnergy_<region>.npz to read
...
# if PATH is a folder, KineticEnergy_<REGION>.npz inside it is used.
