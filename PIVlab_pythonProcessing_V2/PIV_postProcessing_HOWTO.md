...
|---|---|
| `batch_KineticEnergy.ipynb` | For every run: ROI/FULL-averaged kinetic-energy time series `⟨Ek⟩(t)`. Writes a per-run `.npz`, a dataset summary `.csv`, and a resonance figure `⟨Ek⟩,std vs f*`. |
| `batch_KineticEnergyFFT.ipynb` | FFT of `⟨Ek⟩(t)` (average-then-FFT and FFT-then-average). Per-run 2-panel figure + summary. Reads the `.npz` written above. |
| `batch_VelocityFFT.ipynb` | Per-point FFT of U(t), V(t), ROI-averaged amplitude spectra, polarization, dimensionless numbers. Per-run `.npz`, spectrum + polarization figures, summary, and dataset colormaps. |
| `single_KineticEnergy.ipynb` | The kinetic-energy + Ek-FFT work for **one** run; optionally appends its row to the two summaries. |
| `single_KineticEnergyFFT.ipynb` | Viewer: reads one `KineticEnergy_timeSeries` `.npz` and (re)draws its Ek spectrum. |
| `single_VelocityFFT.ipynb` | Viewer: reads one `VelocityFFT` `.npz`, redraws the velocity spectrum + polarization, prints peaks and dimensionless numbers. |
...
3. `batch_KineticEnergy` → `batch_KineticEnergyFFT` (the FFT batch reads the KE
   `.npz`). `batch_VelocityFFT` is independent.
...
|---|---|---|
| `KineticEnergy_timeSeries_<region>.npz` | batch/single KineticEnergy | Ek time series (see §6) |
| `KineticEnergy_summary_<region>.csv` | batch/single KineticEnergy | one row per run |
| `KineticEnergy_vs_<fstar\|flib>_<region>[_normalized].<fmt>` | KineticEnergy / plot_summary | resonance figure |
| `KineticEnergy_FFT_<region>[_normalized].<fmt>` | batch/single KineticEnergyFFT | Ek time series + spectra |
| `KineticEnergyFFT_summary_<region>.csv` | batch KineticEnergyFFT | one row per run |
| `VelocityFFT_<region>.npz` | batch VelocityFFT | velocity spectra + polarization (see §6) |
| `VelocityFFT_<region>[_normalized].<fmt>` | batch/single VelocityFFT | amplitude spectrum figure |
| `Velocity_polarization_<region>[_normalized].<fmt>` | batch/single VelocityFFT | polarization figure |
| `VelocityFFT_summary_<region>.csv` | batch VelocityFFT | one row per run |
| `VelocityFFT_colormap_*_<region>[_normalized].<fmt>` | batch VelocityFFT | sweep colormaps (vs f*, δφ, f_lib) |
...
### `KineticEnergy_timeSeries_<region>.npz`
...
- **KineticEnergy:** `mean_Ekin`, `std_Ekin`, `mean_Ekin_star`, `std_Ekin_star`.
- **KineticEnergyFFT:** `f_peak_Hz`, `f_peak_star`, `amp_peak`, `amp_peak_star`,
  and the `_perPoint` variants.
