...
from piv_common import (PIV_FILENAME, amp_from_rfft, compute_fft, create_mask,
                        extract_roi, load_piv, parse_run_name, peak_freq)
...
def process_run(run_dir, reprocess=False):
    ...
    print("  [ok] %-40s nframes=%d fps=%.4gHz  f_peak=%.4gHz  "
          "fpeak_freq(per-point)=%.4gHz  npts=%s"
